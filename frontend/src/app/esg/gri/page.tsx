...
function GriFacilitySelectorContent() {
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {facilities.map((facility) => (
              <Card
                ...
                role="button"
                tabIndex={0}
                onClick={() => router.push(`/facilities/${facility.id}/gri/new`)}
                onKeyDown={(e) => {
                  if (e.key === "Enter") router.push(`/facilities/${facility.id}/gri/new`);
                }}
...
export default function GriFacilitySelectorPage() {
  return (
    <ProtectedRoute>
      <GriFacilitySelectorContent />
    </ProtectedRoute>
  );
}
