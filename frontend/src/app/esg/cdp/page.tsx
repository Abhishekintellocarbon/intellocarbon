...
import { ArrowLeft, Factory, Loader2, MapPin, Plus, ScrollText } from "lucide-react";
...
function CdpFacilitySelectorContent() {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold">CDP Climate Change response</h1>
            <p className="text-sm text-muted-foreground">
              Pick a facility. The full questionnaire — C0 through C12 and C15 — with your emissions, energy and
              Scope 3 figures reused automatically.
            </p>
          </div>
        </div>

        <Card className="mb-8 border-teal-500/30 p-6">
          <h2 className="font-medium">CDP is requested by a buyer, not required by law</h2>
        ...
        {facilities && facilities.length === 0 && (
          <Card className="flex flex-col items-center gap-3 p-12 text-center">
            ...
            <p className="max-w-sm text-sm text-muted-foreground">
              Add a facility first — CDP responses are prepared per facility, and reuse that facility&apos;s existing
              emissions, energy and Scope 3 data automatically.
            </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {facilities.map((facility) => (
              <Card
                ...
                role="button"
                tabIndex={0}
                onClick={() => router.push(`/facilities/${facility.id}/cdp/new`)}
                onKeyDown={(e) => {
                  if (e.key === "Enter") router.push(`/facilities/${facility.id}/cdp/new`);
                }}
              ...
              >
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  Already have a CDP response for this facility?{" "}
                  <Link
...
export default function CdpFacilitySelectorPage() {
  return (
    <ProtectedRoute>
      <CdpFacilitySelectorContent />
    </ProtectedRoute>
  );
}
