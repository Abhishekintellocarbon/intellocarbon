...
function IssbFacilitySelectorContent() {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold">ISSB IFRS S1 & S2 disclosure</h1>
            <p className="text-sm text-muted-foreground">
              Pick a facility to disclose Governance, Strategy, Risk Management, and Metrics & Targets for.
            </p>
          </div>
        </div>
        ...
        {facilities && facilities.length === 0 && (
          <Card className="flex flex-col items-center gap-3 p-12 text-center">
            ...
            <p className="max-w-sm text-sm text-muted-foreground">
              Add a facility first — ISSB disclosures are entered per facility, and reuse that facility&apos;s
              existing GHG activity data automatically for Scope 1 and 2.
            </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {facilities.map((facility) => (
              <Card
                ...
                role="button"
                tabIndex={0}
                onClick={() => router.push(`/facilities/${facility.id}/issb/new`)}
                onKeyDown={(e) => {
                  if (e.key === "Enter") router.push(`/facilities/${facility.id}/issb/new`);
                }}
...
export default function IssbFacilitySelectorPage() {
  return (
    <ProtectedRoute>
      <IssbFacilitySelectorContent />
    </ProtectedRoute>
  );
}
