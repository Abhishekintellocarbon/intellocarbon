...
import { griReportingPeriodSchema } from "@/lib/validations/gri";
...
function NewGriReportContent() {
  ...
  const handleContinue = () => {
    const result = griReportingPeriodSchema.safeParse(draftPeriod);
    if (!result.success) {
      setPeriodError(result.error.issues[0]?.message ?? "Enter a valid reporting period");
      return;
    }
    setPeriodError(null);
    // Straight to materiality, not to the disclosure form. GRI 3 is the
    ...
    // dead-end them.
    router.push(`/facilities/${params.id}/gri/${encodeURIComponent(result.data)}/materiality`);
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold">Start a GRI Standards report</h1>
            <p className="text-sm text-muted-foreground">
              Full GRI Standards 2021 — General Disclosures, a materiality assessment, and the Topic Standards it
              determines apply.
            </p>
          </div>
        </div>
        ...
        <Card className="max-w-sm p-6">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            You&apos;ll start with the GRI 3 materiality assessment, which determines which Topic Standards you
            report.
          </p>
...
export default function NewGriReportPage() {
  return (
    <ProtectedRoute>
      <NewGriReportContent />
    </ProtectedRoute>
