...
import { ApiError, csrdApi } from "@/lib/api";
import type { CsrdReport } from "@/lib/types";
 ...
 * for those companies.
 */
export function CsrdSection({ facilityId }: { facilityId: string }) {
  const [reports, setReports] = useState<CsrdReport[] | null>(null);
  ...
  return (
    <>
      <div className="mt-10 mb-4 flex items-center justify-between">
        ...
        <Link href={`/facilities/${facilityId}/csrd/new`}>
      ...
      {reports === null ? (
        ...
        <Card className="flex flex-col items-center gap-3 p-12 text-center">
          ...
          <Link href={`/facilities/${facilityId}/csrd/new`} className="mt-2">
            <Button size="sm" variant="secondary">
              <Plus className="h-4 w-4" />
              Start GRI report
            </Button>
        ...
        <div className="space-y-3">
          {reports.map((report) => {
            ...
            const href = !materialityComplete
              ? `/facilities/${facilityId}/csrd/${encodeURIComponent(report.reportingPeriod)}/materiality`
              : isDraft
                ? `/facilities/${facilityId}/csrd/${encodeURIComponent(report.reportingPeriod)}/edit`
                : `/facilities/${facilityId}/csrd/${encodeURIComponent(report.reportingPeriod)}`;
              ...
              <Link key={report.id} href={href}>
                <Card className="flex flex-col gap-3 p-5 transition-colors hover:border-teal-500/40 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <span className="text-xs text-muted">
                    {!materialityComplete
                      ? "Double materiality assessment not finished"
                      : isDraft
                        ? `${materialCount} material standards · not yet submitted`
                        : `${materialCount} material standards · view statement & download PDF`}
                  </span>
                </Card>
              </Link>
            );
